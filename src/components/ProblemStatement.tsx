...
export default function ProblemStatement() {
  ...
  return (
    <section id="problem" className="relative py-24 bg-navy-950/40 border-t border-white/5 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-16">
            ...
            <div className="space-y-4 font-sans text-sm text-slate-400">
              {PROBLEM_STATEMENT.challenges.map((challenge, idx) => (
                <div key={challenge.id} className="flex gap-3">
                  ...
                  <div>
                    <strong className="text-slate-200 block mb-0.5">{challenge.title}</strong>
                    <span className="text-xs text-slate-400 leading-normal">{challenge.desc}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="lg:col-span-6">
            <div className="p-6 rounded-3xl bg-slate-900/80 border border-white/5 relative overflow-hidden backdrop-blur-md">
              ...
              <h4 className="text-xs font-mono font-bold text-slate-505 text-slate-400 text-left mb-6 uppercase tracking-wider">
                Legacies GPS Signal Loss Mechanism
              </h4>

              {/* Vector diagram showing Satellites bouncing off concrete block */}
              <div className="h-[200px] relative border border-white/5 bg-slate-950 rounded-xl overflow-hidden flex items-center justify-center">
                ...
                <div className="absolute top-[45%] left-1/2 -translate-x-1/2 w-44 p-2 bg-slate-900 border border-slate-700/60 rounded-lg text-center shadow-lg">
                  <Building2 size={16} className="text-slate-400 mx-auto mb-1 animate-pulse" />
                  <div className="text-[9px] font-mono font-medium text-slate-200">MSRIT Apex Block Roof</div>
        ...
        <div className="text-center py-4 text-slate-400 text-sm italic font-sans mb-8 border-t border-b border-white/5 bg-slate-900/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {LATENCY_STATS.map((stat) => (
            <div 
              key={stat.id}
              className="p-6 rounded-2xl bg-slate-900/60 border border-white/5 hover:border-cyan-500/30 transition duration-300 flex flex-col justify-between text-left relative overflow-hidden group"
            >
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <div className="p-2 ml-0 rounded-lg bg-slate-950 border border-white/10">
                    {getIcon(stat.icon)}
                  </div>
                ...
                <div className="space-y-1">
                  <div className="text-3xl font-display font-bold text-white tracking-tight group-hover:text-rose-400 transition-colors duration-300">
                    {stat.value}
                  </div>
                  <div className="text-xs font-semibold text-slate-300 font-display">
                    {stat.label}
                  </div>
                </div>
              </div>
